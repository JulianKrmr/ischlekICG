...
window.addEventListener("load", () => {
  ...
  function animate() {
    ...
    const sphere = new Sphere(
      matrix.mulVec(new Vector(0.1, 0, -1.5, 1)),
      0.4,
      new Vector(0.3, 0, 0, 1)
    );
  ...
  const useRotationElement = document.getElementById(
    "userotation"
  ) as HTMLInputElement;
  useRotationElement.onchange = () => {
    ...
    if (useRotationElement.checked) {
      range.value = "0";
      range.oninput = () => {
        rotation = Matrix.rotation(new Vector(0, 0, 1, 0), Number(range.value));
        window.requestAnimationFrame(animate);
      };
  ...
  const useTranslationElement = document.getElementById(
    "usetranslation"
  ) as HTMLInputElement;
    ...
    if (useTranslationElement.checked) {
      ...
      range.oninput = () => {
        translation = Matrix.translation(
          new Vector(Number(range.value), 0, 0, 0)
        );
        window.requestAnimationFrame(animate);
      };
      range.disabled = false;
  ...
  const useScaleElement = document.getElementById(
    "usescale"
  ) as HTMLInputElement;
